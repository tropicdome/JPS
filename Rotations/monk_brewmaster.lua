function monk_brewmaster(self)
   ...
   -- Correct "Expel harm" according to health pool at level 90
	...
	local spellTable =
	{
		...
		{ "Touch of Death",			UnitHealth("target") <= UnitHealth("player") and chi >= 3 }, 
		-- AoE
--		{ "Chi Wave",				jps.MultiTarget },		
		{ "Breath of Fire",			jps.MultiTarget and chi >= 2 },
		{ "Rushing Jade Wind",		jps.MultiTarget },
		{ "Spinning Crane Kick",	jps.MultiTarget and energy >= 40 and CheckInteractDistance("target",3) ~= nil },
		-- Single target
		-- Chi Builders
		{ "Keg Smash",				chi < 3 and CheckInteractDistance("target",3) ~= nil },
		...
		{ "Tiger Palm",				jps.buffDuration("Tiger Power") <= 2.5 or jps.buffStacks("Tiger Power") < 3 }, -- No chi cost due to Brewmaster specialization at level 34
		{ "Blackout Kick",			"onCD" },
		{ "Chi Brew" , 				chi == 0 },	-- If Chi Brew talent instead of Power Strikes. Almost equal in Chi generation
	}
